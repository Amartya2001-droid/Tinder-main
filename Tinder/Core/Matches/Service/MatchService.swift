...
struct MatchService: MatchServiceProtocol {
    ...
    func saveMatch(withUser user: User) async throws {
        ...
        let userMatchRef = FirestoreConstants.UserCollection.document(user.id).collection("user-matches").document(currentUserMatchRef.documentID)
        let userMatch = Match(id: currentUserMatchRef.documentID, userId: currentUid, matchTimestamp: timestamp)
